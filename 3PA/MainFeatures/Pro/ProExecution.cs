...
namespace _3PA.MainFeatures.Pro {
    ...
    internal abstract class ProExecutionBasic {

        #region Factory

        public static ProExecutionBasic Factory(ExecutionType executionType) {
        ...
        /// </summary>
        public Action<ProExecutionBasic> OnExecutionEnd { private get; set; }
        ...
        /// </summary>
        public Action<ProExecutionBasic> OnExecutionOk { private get; set; }

        /// <summary>
        /// The action to execute at the end of the process if something went wrong (no .log or database down)
        /// </summary>
        public Action<ProExecutionBasic> OnExecutionFailed { private get; set; }
        ...
        public bool Do() {
            ...
            _localTempDir = Path.Combine(Config.FolderTemp, DateTime.Now.ToString("MMdd_HHmmss") + "-" + Path.GetRandomFileName());
            ...
            var runnerPath = Path.Combine(_localTempDir, "run_" + DateTime.Now.ToString("yyMMdd_HHmmssfff") + ".p");
    ...
    internal abstract class ProExecutionHandleCompilation : ProExecutionBasic {
        ...
        protected override bool SetExecutionInfo() {
            ...
            foreach (var fileToCompile in Files) {
                ...
                if (fileToCompile.InputPath.Equals(Npp.CurrentFile.Path) &&
                    ...
                    fileToCompile.CompInputPath = Path.Combine(_localTempDir, "tmp_" + DateTime.Now.ToString("yyMMdd_HHmmssfff_") + count + Path.GetExtension(fileToCompile.InputPath));
                ...
                if ((lastDeployment.DeployType != DeployType.Move) ||
                    ...
                    if (lastDeployment.DeployType != DeployType.Ftp && !string.IsNullOrEmpty(ProEnv.BaseCompilationPath) && ProEnv.BaseCompilationPath.Length > 2 && !ProEnv.BaseCompilationPath.Substring(0, 2).EqualsCi(_localTempDir.Substring(0, 2))) {
                        DistantTempDir = Path.Combine(ProEnv.BaseCompilationPath, DateTime.Now.ToString("MMdd_HHmmss") + "-" + Path.GetRandomFileName());
            ...
            ProgressionFilePath = Path.Combine(_localTempDir, "compile.progression");
            _compilationLog = Path.Combine(_localTempDir, "compilation.log");


            SetPreprocessedVar("CompileWithDebugList", CompileWithDebugList.ToString());
            SetPreprocessedVar("ToCompileListFile", filesListPath.ProQuoter());
            SetPreprocessedVar("CompileProgressionFile", ProgressionFilePath.ProQuoter());
        ...
        public List<FileToDeploy> CreateListOfFilesToDeploy() {
            ...
            foreach (var treatedFile in Files) {
    ...
    internal class ProExecutionProlint : ProExecutionHandleCompilation {
        ...
        protected override bool SetExecutionInfo() {
            ...
            ProlintOutputPath = Path.Combine(_localTempDir, "prolint.log");
        ...
        protected override Dictionary<string, List<FileError>> GetErrorsList(Dictionary<string, string> changePaths) {
            ...
            if (!changePaths.ContainsKey(treatedFile.CompInputPath))
                changePaths.Add(treatedFile.CompInputPath, treatedFile.InputPath);
            return FilesInfo.ReadErrorsFromFile(ProlintOutputPath, true, changePaths);
        }
    }

    internal class ProExecutionDatabase : ProExecutionBasic {
    ...
    internal class ProExecutionAppbuilder : ProExecutionBasic {
    ...
    internal class ProExecutionDictionary : ProExecutionBasic {
        public override ExecutionType ExecutionType { get { return ExecutionType.Dictionary; } }
    }

    internal class ProExecutionDataDigger : ProExecutionBasic {
    ...
    internal class ProExecutionDbAdmin : ProExecutionBasic {
        public override ExecutionType ExecutionType { get { return ExecutionType.DbAdmin; } }
    }

    internal class ProExecutionProDesktop : ProExecutionBasic {
        public override ExecutionType ExecutionType { get { return ExecutionType.ProDesktop; } }
    }

    internal class ProExecutionDeploymentHook : ProExecutionBasic {
        ...
        public string DeploymentStep { get; set; }
    }
    ...
    internal class ProExecution {
        ...
        ~ProExecution() {
            try {
                ...
                ErrorHandler.LogError(e, "Proexecution destructor");
            }
        }
        ...
        public bool Do(ExecutionType executionType) {
            ...
            if (executionType == ExecutionType.Database)
                ExtractDbOutputPath = Path.Combine(LocalTempDir, "db.extract");
            ...
            runnerProgram.AppendLine("&SCOPED-DEFINE LogPath " + LogPath.ProQuoter());
            ...
            runnerProgram.AppendLine("&SCOPED-DEFINE CompileWithDebugList " + ProEnv.CompileWithListing);
            runnerProgram.AppendLine("&SCOPED-DEFINE ToCompileListFile " + filesListPath.ProQuoter());
            runnerProgram.AppendLine("&SCOPED-DEFINE DbLogPath " + DatabaseConnectionLog.ProQuoter());
