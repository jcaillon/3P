    ...
    internal class ProExecution {
        ...
        public bool Do(ExecutionType executionType) {
            ...
            foreach (var fileToCompile in ListToCompile) {
                ...
                var lastDeployment = ProEnv.GetDeployNeeded(fileToCompile.InputPath).Last();
                ...
                if (lastDeployment.DeployType != DeployType.Copy ||
        ...
        public List<FileToDeploy> CreateListOfFilesToDeploy() {
            ...
            foreach (var treatedFile in ListToCompile) {
                ...
                if (treatedFile.InputPath.EndsWith(".cls", StringComparison.CurrentCultureIgnoreCase)) {
                    ...
                    if (listOfRFiles != null) {
                        ...
                        foreach (var file in listOfRFiles) {
                            ...
                            if (string.IsNullOrEmpty(sourcePath)) {
                                ...
                                foreach (var deployNeeded in ProEnv.GetDeployNeeded(sourcePath)) {
                                    string outputRPath;
                                    ...
                                    outputList.Add(new FileToDeploy(treatedFile.InputPath, file, outputRPath, deployNeeded));
                                    ...
                                    if (ProEnv.CompileWithListing && Path.GetFileNameWithoutExtension(relativePath).Equals(Path.GetFileNameWithoutExtension(treatedFile.InputPath))) {
                                        outputList.Add(new FileToDeploy(treatedFile.InputPath, treatedFile.CompOutputLst, Path.ChangeExtension(outputRPath, ".lst"), deployNeeded));
                                    }
                                }
                            }
                        }
                    ...
                    foreach (var deployNeeded in ProEnv.GetDeployNeeded(treatedFile.InputPath)) {
                        // add .r and .lst (if needed) to the list of files to deploy
                        outputList.Add(new FileToDeploy(treatedFile.InputPath, treatedFile.CompOutputR, Path.Combine(deployNeeded.TargetDir, treatedFile.BaseFileName + ".r"), deployNeeded));

                        if (ProEnv.CompileWithListing) {
                            outputList.Add(new FileToDeploy(treatedFile.InputPath, treatedFile.CompOutputLst, Path.Combine(deployNeeded.TargetDir, treatedFile.BaseFileName + ".lst"), deployNeeded
                                ));
                        }
                    }
                }
