...
namespace _3PA.MainFeatures.Pro {
        ...
        protected override bool SetExecutionInfo() {
            ...
            foreach (var fileToCompile in Files) {
                ...
                if (fileToCompile.SourcePath.Equals(Npp.CurrentFile.Path) &&
                    ...
                    fileToCompile.CompiledSourcePath = Path.Combine(_localTempDir, "current_file_" + DateTime.Now.ToString("HHmmssfff") + Path.GetExtension(fileToCompile.SourcePath));
                ...
                var baseFileName = Path.GetFileNameWithoutExtension(fileToCompile.CompiledSourcePath);
        ...
        protected override void PublishExecutionEndEvents() {
            ...
            if (!ExecutionFailed && (!ConnectionFailed || !NeedDatabaseConnection)) {
                ...
                if (OnCompilationOk != null) {
                    OnCompilationOk(this, Files, LoadErrorLog(), GetFilesToDeployAfterCompilation());
                }

                if (OnEachCompilationOk != null) {
                    OnEachCompilationOk(this, Files, LoadErrorLog(), GetFilesToDeployAfterCompilation());
                }
            }            
    ...
    internal class ProExecutionCompile : ProExecutionHandleCompilation {
        ...
        /// <summary>
        /// Deploy the files generated during this compilation (.r and .dbg, .lis...)
        /// </summary>
        public List<FileToDeploy> DeployCompiledFiles() {
            return ProEnv.Deployer.DeployFiles(GetFilesToDeployAfterCompilation(), null, null);
        }
